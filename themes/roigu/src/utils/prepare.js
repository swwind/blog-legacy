...
const counter = (path, increase = false) => {
  if (increase) {
    return fetch(window.APIPrefix + '/count?url=' + path.map(url2base64).join(',')).then(res => res.json());
  } else {
    return fetch(window.APIPrefix + '/query?url=' + path.map(url2base64).join(',')).then(res => res.json());
  }
}
